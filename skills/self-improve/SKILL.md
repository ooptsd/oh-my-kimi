...
- New runs default to `.omk/self-improve/topics/default/`.
- When the user provides a topic or slug, use `.omk/self-improve/topics/{topic_slug}/`.
- Legacy single-track state at `.omk/self-improve/` remains valid only as a compatibility fallback when no explicit topic/slug is supplied and that flat layout already exists.
...
OMC mode lifecycle: `.omk/state/sessions/{sessionId}/self-improve-state.json`
