...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `${CLAUDE_CONFIG_DIR:-~/.claude}/.omk-config.json`.

### Step 1: Detect Existing Configuration

```bash
CONFIG_FILE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.omk-config.json"
...
Config saved to: ~/.claude/.omk-config.json
...
Env vars are auto-detected by the notification system without needing `.omk-config.json`.
...
Build the notifications object with the collected values and merge into `.omk-config.json` using jq:
...
- `omk --telegram` — Activates Telegram notifications (sets `OMC_TELEGRAM=1`)
- `omk --discord` — Activates Discord notifications (sets `OMC_DISCORD=1`)
- `omk --slack` — Activates Slack notifications (sets `OMC_SLACK=1`)
- `omk --webhook` — Activates webhook notifications (sets `OMC_WEBHOOK=1`)
- `omk --openclaw` — Activates OpenClaw gateway integration (sets `OMC_OPENCLAW=1`)
...
**Examples:**
- `omk --telegram --discord` — Telegram + Discord active
- `omk --telegram --slack --webhook` — Telegram + Slack + Webhook active
- `omk --telegram --openclaw` — Telegram + OpenClaw active
- `omk` — No notifications sent (all platforms require explicit activation)
