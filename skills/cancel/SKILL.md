...
- **Ultragoal**: Clears the session-scoped ultragoal runtime guard (`.omk/state/.../ultragoal-state.json`) so PreToolUse `/goal` enforcement and Stop reinforcement release. Durable `.omk/ultragoal/` plan/ledger artifacts are preserved.
...
**WARNING:** Do NOT use this fallback for `autopilot` or `omk-teams`. Autopilot requires
`state_write(active=false)` to preserve resume data. omk-teams requires tmux session
...
REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || { d="$PWD"; while [ "$d" != "/" ] && [ ! -d "$d/.omk" ]; do d="$(dirname "$d")"; done; echo "$d"; })"
...
elif [ "$REPO_ROOT" != "/" ] && [ -d "$REPO_ROOT/.omk" ]; then
  OMC_STATE="$REPO_ROOT/.omk/state"
else
  echo "ERROR: Could not locate .omk state directory" >&2
  exit 1
fi
...
4. Ultragoal (standalone runtime guard — `state_clear(mode="ultragoal")`; preserves durable `.omk/ultragoal/` artifacts)
5. Swarm (standalone)
...
11. Self-Improve (standalone — clear state, clean orphaned worktrees, preserve iteration_state for resume, set status: "user_stopped" in the resolved `<self-improve-root>/state/agent-settings.json`; new runs use `.omk/self-improve/topics/<topic-slug>/`, with flat `.omk/self-improve/` retained only for legacy single-track resumes)
...
4. Team artifacts (`~/.claude/teams/*/`, `~/.claude/tasks/*/`, `.omk/state/team-state.json`) are best-effort cleared as part of the legacy fallback.
   - Cancel for native team does NOT affect omk-teams state, and vice versa.
...
- `.omk/state/omk-teams-state.json`
...
3. If a `session_id` was supplied to `/oh-my-claudecode:cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omk/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
```
  ...
  3. Run OMC tmux/CLI orphan scan only for legacy `omk team` / `/omk-teams` workers (see below)
...
Durable `.omk/ultragoal/{brief.md,goals.json,ledger.jsonl}` artifacts are preserved.
...
5. Team artifacts (`~/.claude/teams/*/`, `~/.claude/tasks/*/`, `.omk/state/team-state.json`) remain best-effort cleanup items invoked during the legacy/global pass.
...
| Ralph | No | N/A |
| Ultrawork | No | N/A |
| Ultragoal | Yes (durable plan/ledger under `.omk/ultragoal/`) | Resume via `/ultragoal` / `omk ultragoal complete-goals` |
| Swarm | No | N/A |
| Ultrapilot | No | N/A |
| Pipeline | No | N/A |
...
1. **Check for active MCP workers**: Look for heartbeat files at `.omk/state/team-bridge/{team}/*.heartbeat.json`
2. **Send shutdown signals**: Write shutdown signal files for each active worker
3. **Kill tmux sessions**: Run `tmux kill-session -t omk-team-{team}-{worker}` for each worker
4. **Clean up heartbeat files**: Remove all heartbeat files for the team
5. **Clean up shadow registry**: Remove `.omk/state/team-mcp-workers.json`

### Force Clear Addition

When `--force` is used, also clean up:
```bash
rm -rf .omk/state/team-bridge/       # Heartbeat files
rm -f .omk/state/team-mcp-workers.json  # Shadow registry
# Kill all omk-team-* tmux sessions
tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^omk-team-' | while read s; do tmux kill-session -t "$s" 2>/dev/null; done
```
