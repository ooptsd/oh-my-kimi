...
2. Scan user skills at `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omk-learned/`
...
USER SKILLS (~/.claude/skills/omk-learned/):
...
5. **Ask for scope:**
   - `user` → `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omk-learned/<name>/SKILL.md`
...
✓ Created skill at ~/.claude/skills/omk-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omk-learned/<name>/SKILL.md`
   - `.omk/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omk-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.claude/skills/omk-learned/)
...
```
Skill: custom-logger
Scope: user (~/.claude/skills/omk-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.claude/skills/omk-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.claude/skills/omk-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omk-learned/`
   - Project skills: `.omk/skills/`
...
USER_SKILLS_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omk-learned"
...
echo "=== USER-LEVEL SKILLS (~/.claude/skills/omk-learned/) ==="
if [ -d "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omk-learned" ]; then
  USER_COUNT=$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omk-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omk-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
Then ask for scope:
- **User-level** (~/.claude/skills/omk-learned/) - Available across all projects
...
✓ User skills directory exists: ~/.claude/skills/omk-learned/
