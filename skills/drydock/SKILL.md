...
| Tools (composable capability) | `.omk/skills/` + `.mcp.json` + `scripts/` |
...
- `CONTEXT.md`, `docs/adr/`, `docs/standards/`, `docs/business/`, `design-system/`, `.omk/skills/`, `.mcp.json`, `scripts/`, `.gitattributes` — which exist, which are missing?
...
- Reusable capability goes to .omk/skills/; UI patterns go to design-system/
```
...
- 可复用能力沉淀到 .omk/skills/；UI 模式沉淀到 design-system/
```
...
- 可重用能力沉澱到 .omk/skills/；UI 模式沉澱到 design-system/
```
...
Seed F — .omk/skills/README.md:

````markdown
# Project Skills

Reusable capabilities sedimented by this project: specialized tools, prompt templates, specialized practices.
One skill per file `.omk/skills/<name>.md`, frontmatter must contain a stable ASCII `id` plus name + description +
...
- **anyone** can add a project skill to .omk/skills/ — the barrier is the skillify quality gate, not permission
