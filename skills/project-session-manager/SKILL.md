...
> **Quick Start (worktree-first):** Start with `omk teleport` when you want an isolated issue/PR/feature worktree before adding any tmux/session orchestration:
> ```bash
> omk teleport #123          # Create worktree for issue/PR
> omk teleport my-feature    # Create worktree for feature
> omk teleport list          # List worktrees
> ```
...
|---------|-------------|---------|
| `review <ref>` | PR review session | `/psm review omk#123` |
| `fix <ref>` | Issue fix session | `/psm fix omk#42` |
| `feature <proj> <name>` | Feature development | `/psm feature omk add-webhooks` |
...
Supported formats:
- **Alias**: `omk#123` (requires `~/.psm/projects.json`)
- **Full**: `owner/repo#123`
...
|------|---------------------------------|---------------------------------|--------------|
| PR Review | `omc:pr-123` | `psm_omc_pr-123` | `~/.psm/worktrees/omk/pr-123` |
| Issue Fix | `omc:issue-42` | `psm_omc_issue-42` | `~/.psm/worktrees/omk/issue-42` |
| Feature | `omc:feat-auth` | `psm_omc_feat-auth` | `~/.psm/worktrees/omk/feat-auth` |
...
9. **Output session info**:
   ...
   Session ready!
     ...
     ID: omc:pr-123
     Worktree: ~/.psm/worktrees/omk/pr-123
     Tmux: psm_omc_pr-123
...
4. **Format output**:
   ...
   -------------------|---------|----------|---------------------------
   omc:pr-123        | review  | active   | ~/.psm/worktrees/omk/pr-123
   omc:issue-42      | fix     | detached | ~/.psm/worktrees/omk/issue-42
   ```
...
The `omk teleport` command provides a lightweight alternative to full PSM sessions. It creates git worktrees without tmux session management — ideal for quick, isolated development.
...
omk teleport #123
omk teleport owner/repo#123
omk teleport https://github.com/owner/repo/issues/42

# Create worktree for a feature
omk teleport my-feature

# List existing worktrees
omk teleport list

# Remove a worktree
omk teleport remove issue/my-repo-123
omk teleport remove --force feat/my-repo-my-feature
```
...
| `--path <path>` | Custom worktree root directory | `~/Workspace/omk-worktrees/` |
