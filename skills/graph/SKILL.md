...
`src/graph/*` and executes through an independent OS process (`omk graph run`),
...
The execution surface is always the CLI subcommand:
    ...
    omk graph run <descriptor.json> [--runs-root <dir>]
...
- Auditable runs: OCC journal + projection snapshots under `.omk/graph-runs/<run_id>/`
...
2. **Pipeline described** -> author the descriptor JSON (schema below), write
   it next to the project (suggest `.omk/graphs/<name>.json`) and show it to
...
- **Agent authority boundary**: built-in agent nodes are explicitly read-only.
  ...
  `.omk/graph-runs/<run_id>/descriptor.json` as executable content.
