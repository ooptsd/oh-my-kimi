...
3. **On team cancellation, handoffs survive** in `.omk/handoffs/` for session resume. They are not deleted by native Claude Code team cleanup; no `TeamDelete` call exists in Claude Code 2.1.178+.
...
- **Cancel:** `/oh-my-claudecode:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.omk/handoffs/` are preserved for potential resume.
...
5. **Clean OMC state** -- Remove `.omk/state/team-state.json`
...
- NEVER run team spawning/orchestration skills or commands (for example `$team`, `$autopilot`, `$ralph`, `omk team ...`, `omx team ...`)
...
For legacy OMC tmux/CLI worker runs (`omk team` / `/omk-teams`), check for worker processes that survived cleanup:
...
| ----------------------- | ---------------------------------------------------------------- | -------------------------------------- |
| **Storage**             | OMC state/handoffs plus Claude Code's current task-list surface   | SQLite in `.omk/state/swarm.db`        |
...
Optional settings live in `.claude/omk.jsonc` (project) or `~/.config/claude-omk/config.jsonc` (user). Project values override user values; `OMC_TEAM_ROLE_OVERRIDES` (env JSON) supersedes both.
...
1. Native Claude Code 2.1.178+ has no per-team `TeamDelete` cleanup. After shutdown is confirmed or timed out, clear OMC state via MCP tools:
   ...
   ```
2. For legacy OMC tmux/CLI workers, run the documented `omk team shutdown` / cleanup path.
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.omk/worktrees/{team}/{worker}` with branch `omk-team/{teamName}/{workerName}`.
...
- **Multi-repo workspace anchor:** drop a `.omk-workspace` marker at the parent directory so multiple sessions across sub-repos share one `.omk/`. Resolution order: `OMC_STATE_DIR > .omk-workspace > git > cwd`. See `docs/REFERENCE.md`.
- **Session id source:** OMC_SESSION_ID env var wins in CLI contexts; hook payload data.session_id wins in hook contexts.
- **Plan id (when applicable):** Team state is session-scoped. Team handoffs at `.omk/handoffs/` are shared by design (see Wave G in the workspace plan).
