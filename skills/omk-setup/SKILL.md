...
USAGE:
  /oh-my-kimi:omk-setup           Run initial setup wizard (or update if already configured)
  /oh-my-kimi:omk-setup --local   Configure local project (.claude/CLAUDE.md)
  /oh-my-kimi:omk-setup --global  Configure global settings (~/.claude/CLAUDE.md)
  /oh-my-kimi:omk-setup --force   Force full setup wizard even if already configured
  /oh-my-kimi:omk-setup --help    Show this help
...
MODES:
  ...
  Global Configuration (--global)
    ...
    - Optional preserve mode keeps the user's base `CLAUDE.md` and installs OMC into `CLAUDE-omk.md` for `omk` launches
...
EXAMPLES:
  /oh-my-kimi:omk-setup           # First time setup (or update CLAUDE.md if configured)
  /oh-my-kimi:omk-setup --local   # Update this project
  /oh-my-kimi:omk-setup --global  # Update all projects
  /oh-my-kimi:omk-setup --force   # Re-run full setup wizard
...
CONFIG_FILE="$CONFIG_DIR/.omk-config.json"
...
Read the file at `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omk-setup/phases/01-install-claude-md.md` and follow its instructions.
...
1. **Phase 1 - Install CLAUDE.md**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omk-setup/phases/01-install-claude-md.md` and follow its instructions.

2. **Phase 2 - Environment Configuration**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omk-setup/phases/02-configure.md` and follow its instructions. Phase 2 must delegate HUD/statusLine setup to the `hud` skill; do not generate or patch `statusLine` paths inline here.

3. **Phase 3 - Integration Setup**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omk-setup/phases/03-integrations.md` and follow its instructions.

4. **Phase 4 - Completion**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omk-setup/phases/04-welcome.md` and follow its instructions.
...
**Automatic**: Just run `/oh-my-kimi:omk-setup` - it will detect you've already configured and offer a quick "Update CLAUDE.md and clear retired setup values" option that skips the rest of the wizard.
...
**Manual options**:
- `/oh-my-kimi:omk-setup --local` to update project config only
- `/oh-my-kimi:omk-setup --global` to update global config only
- `/oh-my-kimi:omk-setup --force` to re-run the full wizard (reconfigure preferences)
