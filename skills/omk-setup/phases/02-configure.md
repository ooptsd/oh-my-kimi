...
if ! RESUME_LAST_COMPLETED_STEP=$(jq -r '.lastCompletedStep // 0' ".omk/state/setup-state.json" 2>/dev/null); then
...
Ralph is an agent-driven persistence loop with no external language dependency. Its only hard prerequisites are the Node runtime that already runs OMC's hooks and a writable config directory for `.omk/state/` state files. Verify both without blocking the rest of setup:
...
touch "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.omk-write-probe" 2>/dev/null && rm -f "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.omk-write-probe" \
...
This will:
1. Install the HUD wrapper script to `~/.claude/hud/omk-hud.mjs`
2. Configure `statusLine` in `~/.claude/settings.json`
3. Report status and prompt to restart if needed

After HUD setup completes, save progress:
```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".omk/state/setup-state.json" 2>/dev/null || echo "unknown")
...
const b=p.join(d,'plugins','cache','omk','oh-my-claudecode');
...
// Try .omk-version.json second
if(v==='')try{const j=JSON.parse(f.readFileSync('.omk-version.json','utf-8'));v=j.version||''}catch{}
...
The `ultrawork` workflow was removed in 5.0.0 and the `defaultExecutionMode` config key is no longer read by any runtime surface. Upgrades from 4.x may still carry a dead persisted value in `.omk-config.json`. Clear it so the config matches the current contract:
...
The OMC CLI (`omk` command) provides standalone helper commands such as `omk hud`, `omk teleport`, and `omk team ...`.

First, check if the CLI is already installed:

```bash
if command -v omk &>/dev/null; then
  OMC_CLI_VERSION=$(omk --version 2>/dev/null | head -1 || echo "installed")
...
**Question:** "Would you like to install the OMC CLI globally for standalone helper commands? (`omk`, `omk hud`, `omk teleport`)"
...
if ! command -v npm &>/dev/null; then
  ...
  if npm install -g oh-my-claude-sisyphus 2>&1; then
    ...
    if command -v omk &>/dev/null; then
      OMC_CLI_VERSION=$(omk --version 2>/dev/null | head -1 || echo "installed")
      echo "Verified: omk $OMC_CLI_VERSION"
    else
      echo "Installed but 'omk' not on PATH. You may need to restart your shell."
    fi
  else
