...
omk-plan -> execute -> omk-review -> verify, invoked as /oh-my-kimi:omk-plan and /oh-my-kimi:omk-review.
...
- omk hud         - Render the current HUD statusline
- omk teleport    - Create an isolated git worktree
- omk team status - Inspect a running team job
- Session summaries are written to `.omk/sessions/*.json`
...
/deep-dive, /sciomk, /ccg, /omk-teams, /setup, /mcp-setup, /omk-reference,
/learner, /writer-memory, /local-build-reminder.
Use the replacement instead — see the migration table in docs/MIGRATION.md
(commonly /execute for ultrawork, /verify for ultraqa, /team for omk-teams,
/omk-setup for setup, and Claude Code's native MCP config for mcp-setup;
/wiki for omk-reference).
...
if [ -z "$OMC_VERSION" ]; then
  OMC_VERSION=$(omk --version 2>/dev/null | head -1 || true)
fi
if [ -z "$OMC_VERSION" ]; then
  OMC_VERSION="unknown"
fi
