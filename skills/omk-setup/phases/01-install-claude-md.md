...
1. **Overwrite base CLAUDE.md (Recommended)** - plain `claude` and `omk` both use OMC globally.
2. **Keep base CLAUDE.md; use OMC only through `omk`** - preserve the user's base file, install OMC into `CLAUDE-omk.md`, and let `omk` force-load that companion config at launch.
...
The coordinator exclusively performs all `CLAUDE.md`, `CLAUDE-omk.md`, managed-import, orphan-cleanup, backup, and rollback mutations. Do **not** hand-write, summarize, partially reconstruct, download, or repair either configuration file. It reports only the exact coordinator-created backup, failure, and rollback paths.

After a successful local or global-overwrite install, verify the target file contains both markers. In global preserve mode, verify `CLAUDE-omk.md` contains both markers and the base `CLAUDE.md` contains exactly one managed import block. Stop and report a coordinator failure; never attempt a shell, downloaded-source, or fallback mutation.

For `local` installs inside a git repository, the script also seeds `.git/info/exclude` with an OMC block that re-includes `.omk/`, ignores local `.omk/*` artifacts by default, and preserves `.omk/skills/` for project skills you intend to commit.
...
**Note**: Preserve mode installs OMC into a companion `CLAUDE-omk.md` with a small managed import block, and `omk` launch force-loads that companion config without changing plain `claude`.
...
- Git excludes: Added local `.omk/*` ignore rules to `.git/info/exclude` (keeps `.omk/skills/` trackable for committed project skills)
...
- Companion: May install ~/.claude/CLAUDE-omk.md when preserve mode is chosen
