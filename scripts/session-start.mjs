...
function getRuntimeBaseDir() {
  return process.env.KIMI_PLUGIN_ROOT || join(__dirname, '..');
}
...
function formatUpdateNoticeForUser(updateInfo, options = {}) {
  ...
  const action = updateInfo?.source === 'marketplace'
    ? 'To update the plugin channel, run: /plugin marketplace update omk-plugins && /omk-setup'
  ...
  return `[OMK UPDATE AVAILABLE] oh-my-claudecode v${latestVersion} is available (current: v${currentVersion}). ${action}`;
}
...
async function main() {
  try {
    ...
    if (process.env.KIMI_PLUGIN_ROOT) {
      ...
      publishCacheOccupancy(process.env.KIMI_PLUGIN_ROOT, configDir, ownerPid, occupancyResolution.identities.get(ownerPid));
    }
...
${cleanContent}
    ...
    try {
      ...
      const pluginRoot = process.env.KIMI_PLUGIN_ROOT?.replace(/[\/\\]+$/, ''); // strip trailing separators
