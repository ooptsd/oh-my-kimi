 ...
 * and toSafeSkillName (plan -> omk-plan).
 */
 ...
 * yet `skills/plan` exists (registering `omk-plan`). Bare names therefore
...
function resolveBundledSkill(subagentType, directory) {
  ...
  // claim (e.g. the plan/ dir whose frontmatter registers as omk-plan).
...
const SKILL_PROTECTION_MAP = {
  ...
  'omk-teams': 'none', cancel: 'none',

  // === Instant / read-only → no protection needed ===
  trace: 'none', hud: 'none', 'omk-doctor': 'none', 'omk-help': 'none',
  ...
  sciomk: 'medium', learner: 'medium', 'omk-setup': 'medium',
  setup: 'medium',        // alias for omk-setup
...
function writeSkillActiveState(stateDir, skillName, sessionId, rawSkillName) {
  ...
  // Nesting guard: when a skill (e.g. omk-setup) invokes a child skill
