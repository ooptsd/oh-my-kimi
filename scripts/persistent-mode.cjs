    ...
    if (omcTeams.state?.active && !isStaleState(omcTeams.state) && isSessionMatch(omcTeams.state, sessionId)) {
      ...
      if (phase) {
        ...
        if (newCount <= 20) {
          ...
          // Fire-and-forget notification
          sendStopNotification('omk-teams', omcTeams.state, sessionId, directory).catch(() => {});

          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[OMK TEAMS - Phase: ${phase}] OMC Teams workers active. Continue working. When all workers complete, run /oh-my-claudecode:cancel to cleanly exit. If cancel fails, retry with /oh-my-claudecode:cancel --force.`,
            }),
          );
          return;
        }
