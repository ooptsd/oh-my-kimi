...
function buildPreemptiveContextMessage(percentUsed, severity) {
  if (severity === 'critical') {
    return `[OMK CRITICAL] Context at ${percentUsed}% (critical threshold: ${getPreemptiveCriticalThreshold()}%). Run /compact now before continuing with more tools or agent fan-out.`;
  }

  return `[OMK WARNING] Context at ${percentUsed}% (warning threshold: ${getPreemptiveWarningThreshold()}%). Plan a /compact soon to preserve room for the next large tool output.`;
}
...
function isConsensusPlanningSkillInvocation(skillName, toolInput) {
  ...
  if (skillName !== 'plan' && skillName !== 'omk-plan') return false;
